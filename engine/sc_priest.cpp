...
namespace   // ANONYMOUS NAMESPACE ==========================================
{
    ...
    double priest_spell_t::haste() SC_CONST
    {
        ...
        {
            h *= 1.0 / (1.0 + p->talents.enlightenment * 0.03);  // Triumvirate: 3/6/9% (was miscoded 1/2/3%, should've been 2/4/6%)
        }
        return h;
    }
    ...
    void priest_spell_t::player_buff()
    {
        ...
        {
            player_multiplier *= 1.0 + p->talents.focused_power * 0.03;  // Triumvirate: 3/6% (was 2/4%)
        }
    }
    ...
    struct devouring_plague_t : public priest_spell_t
    {
            ...
            priest_spell_t("devouring_plague", player, SCHOOL_SHADOW, TREE_SHADOW), devouring_plague_burst(0)
        {
            ...
            base_cost *= 1.0 - ((p->talents.mental_agility * 0.05) + p->talents.shadow_focus * 0.03);
            ...
            base_hit += p->talents.shadow_focus * 0.01;
            base_crit += p->talents.mind_melt * 0.04;  // Triumvirate: Devouring Plague is periodic, 4/8% (was 3/6%)
    ...
    struct mind_blast_t : public priest_spell_t
    {
            ...
            priest_spell_t("mind_blast", player, SCHOOL_SHADOW, TREE_SHADOW)
        {
            ...
            base_cost *= 1.0 - (p->talents.focused_mind * 0.05 +
                p->talents.shadow_focus * 0.03 +
                p->set_bonus.tier7_2pc_caster() * 0.10);
            base_cost = floor(base_cost);
            base_multiplier *= 1.0 + p->talents.darkness * p->constants.darkness_value;
            base_hit += p->talents.shadow_focus * 0.01;
            base_crit += p->talents.mind_melt * 0.03;  // Triumvirate: Mind Blast is direct, 3/6% (was 2/4%)
            direct_power_mod *= 1.0 + p->talents.misery * p->constants.misery_power_mod;

            base_crit_bonus_multiplier *= 1.0 + p->talents.shadow_power * 0.25;
        ...
        virtual void player_buff()
        {
            ...
            {
                if (p->dots_shadow_word_pain->ticking()) player_multiplier *= 1.0 + p->talents.twisted_faith * 0.03;
            }
        }
    };
    ...
    struct mind_flay_tick_t : public priest_spell_t
    {
            ...
            priest_spell_t("mind_flay", player, SCHOOL_SHADOW, TREE_SHADOW)
        {
            ...
            base_multiplier *= 1.0 + (p->talents.darkness * p->constants.darkness_value +
                p->talents.twin_disciplines * p->constants.twin_disciplines_value);
            base_crit += p->talents.mind_melt * 0.03;  // Triumvirate: Mind Flay tick counted as direct-group per changelog, 3/6% (was 2/4%)
    ...
    struct mind_nuke_t : public priest_spell_t
    {
            ...
            priest_spell_t("mind_nuke", player, SCHOOL_SHADOW, TREE_SHADOW), mb_wait(0), swp_refresh(0), devious_mind_priority(0)
        {
            ...
            base_multiplier *= 1.0 + (p->talents.darkness * p->constants.darkness_value +
                p->talents.twin_disciplines * p->constants.twin_disciplines_value);
            base_crit += p->talents.mind_melt * 0.03;  // Triumvirate: Mind Sear is direct-group per changelog, 3/6% (was 2/4%)
    ...
    struct shadow_word_death_t : public priest_spell_t
    {
            ...
            priest_spell_t("shadow_word_death", player, SCHOOL_SHADOW, TREE_SHADOW), mb_min_wait(0), mb_max_wait(0), devious_mind_filler(0)
        {
            ...
            base_cost *= 1.0 - ((p->talents.mental_agility * 0.05) +
                p->talents.shadow_focus * 0.03);
            base_cost = floor(base_cost);
            base_multiplier *= 1.0 + p->talents.darkness * p->constants.darkness_value +
                p->talents.twin_disciplines * p->constants.twin_disciplines_value;
    ...
    struct shadow_word_pain_t : public priest_spell_t
    {
            ...
            priest_spell_t("shadow_word_pain", player, SCHOOL_SHADOW, TREE_SHADOW), shadow_weaving_wait(0)
        {
            ...
            base_hit += p->talents.shadow_focus * 0.01;
            base_crit += p->talents.mind_melt * 0.04;  // Triumvirate: Shadow Word Pain is periodic, 4/8% (was 3/6%)
            base_crit += p->set_bonus.tier10_2pc_caster() * 0.05;
        ...
        virtual bool ready()
        {
            ...
            if (shadow_weaving_wait)
                if (p->talents.shadow_weaving)
                    if (p->buffs_shadow_weaving->check() < 10)  // Triumvirate: 10 max stacks (was 5)
                        return false;
    ...
    struct vampiric_touch_t : public priest_spell_t
    {
            ...
            priest_spell_t("vampiric_touch", player, SCHOOL_SHADOW, TREE_SHADOW)
        {
            ...
            base_cost *= 1.0 - p->talents.shadow_focus * 0.03;
            base_cost = floor(base_cost);
            base_multiplier *= 1.0 + p->talents.darkness * p->constants.darkness_value;
            base_hit += p->talents.shadow_focus * 0.01;
            base_crit += p->talents.mind_melt * 0.04;  // Triumvirate: Vampiric Touch is periodic, 4/8% (was 3/6%)
            base_crit += p->set_bonus.tier10_2pc_caster() * 0.05;
...
void priest_t::init_glyphs()
{
    ...
    for (int i = 0; i < num_glyphs; i++)
    {
        ...
        else if (n == "fortitude");  // Triumvirate: reworked to +20% effectiveness, but only affects raid recipients, not caster's own DPS - no-op by design
...
void priest_t::init_base()
{
    ...
    attribute_multiplier_initial[ATTR_SPIRIT] *= 1.0 + talents.enlightenment * 0.03;  // Triumvirate: 3/6/9% (was 2/4/6%)
...
void priest_t::init_buffs()
{
    ...
    buffs_shadow_weaving = new buff_t(this, "shadow_weaving", 10, 20.0, 0.0, talents.shadow_weaving / 3.0);  // Triumvirate: 10 stacks (was 5), 20s (was 15s)
