...
double rating_t::get_attribute_base( sim_t* sim, int level, int class_type, int race, int stat_type )
{
  ...
  // NOTE: this engine's class_stats[] tables were only ever populated for
  // the WotLK 60-80 range (retail simc never needed below 60). There is no
  // real classic/TBC per-level attribute curve here, since vanilla scaling
  // is meaningfully non-linear and unrelated to the WotLK curve. For a
  // level-60-cap server (Triumvirate) we still want *something* usable for
  // pre-60 test sims, so levels 1-59 are approximated via backward linear
  // extrapolation from the level 60->61 slope already present in each
  // table. This is a stopgap, not period-accurate data -- replace with a
  // real per-level table if/when sub-60 accuracy actually matters.
  if ( level > 80 )
  {
    return 0.0;
  }
  ...
  if ( level < 60 )
  {
    // stat_list[0] is level 60, stat_list[1] is level 61 in every table.
    double base60 = stat_list[ 0 ].stats[ stat_type ];
    double base61 = stat_list[ 1 ].stats[ stat_type ];
    double slope  = base61 - base60;
    class_value = base60 - slope * ( 60 - level );
    if ( class_value < 0 ) class_value = 0.0;
  }
  else
  {
