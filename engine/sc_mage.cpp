...
namespace { // ANONYMOUS NAMESPACE ==========================================
    ...
    static void trigger_fingers_of_frost(spell_t* s)
    {
        ...
        p->buffs_fingers_of_frost->trigger(3);  // Triumvirate: 3 charges (was 2)
    }
    ...
    void mage_spell_t::player_buff()
    {
            ...
            school == SCHOOL_FROSTFIRE)
        {
            ...
            fire_power_multiplier = p->talents.fire_power * 0.03;  // Triumvirate: 3/6/9/12/15% (was 2/4/6/8/10%)
    ...
    struct arcane_barrage_t : public mage_spell_t
    {
            ...
            mage_spell_t("arcane_barrage", player, SCHOOL_ARCANE, TREE_ARCANE)
        {
            ...
            base_cost *= 1.0 - p->talents.precision * 0.03;
            base_cost *= 1.0 - p->talents.arcane_focus * 0.03;
            ...
            base_crit += p->talents.arcane_instability * 0.02;
    ...
    struct fire_ball_t : public mage_spell_t
    {
            ...
            frozen(-1), ghost_charge(-1)
        {
            ...
            base_crit += p->talents.improved_scorch * 0.02;
    ...
    struct frost_bolt_t : public mage_spell_t
    {
            ...
            mage_spell_t("frost_bolt", player, SCHOOL_FROST, TREE_FROST), frozen(-1)
        {
            ...
            base_multiplier *= 1.0 + p->talents.piercing_ice * 0.03;
            base_multiplier *= 1.0 + p->talents.arctic_winds * 0.01;
            base_multiplier *= 1.0 + p->talents.arcane_instability * 0.02;
            base_multiplier *= 1.0 + p->talents.chilled_to_the_bone * 0.02;
            base_crit += p->talents.arcane_instability * 0.02;
            direct_power_mod += p->talents.empowered_frost_bolt * 0.10;  // Triumvirate: 10/20% (was 5/10%)
...
void mage_t::init_glyphs()
{
    ...
    for (int i = 0; i < num_glyphs; i++)
    {
        ...
        // To prevent warnings....
        else if (n == "arcane_intellect");  // Triumvirate: reworked to +20% effectiveness, but only affects raid recipients, not caster's own DPS - no-op by design
...
void mage_t::init_base()
{
    ...
    attribute_multiplier_initial[ATTR_INTELLECT] *= 1.0 + talents.arcane_mind * 0.04;  // Triumvirate: 4/8/12/16/20% (was 3/6/9/12/15%)
    attribute_multiplier_initial[ATTR_SPIRIT] *= 1.0 + talents.student_of_the_mind * 0.05;  // Triumvirate: 5/10/15% (was miscoded 4/7/10%, should've been 4/8/12%)

    base_spell_crit += talents.pyromaniac * 0.01;
    base_spell_crit += talents.arcane_instability * 0.02;

    initial_spell_power_per_intellect = talents.mind_mastery * 0.04;  // Triumvirate: 4/8/12/16/20% (was 3/6/9/12/15%)
...
void mage_t::regen(double periodicity)
{
    ...
    mana_regen_while_casting += talents.arcane_meditation * 0.25;  // Triumvirate: 25/50/75% (was 17/33/50%)
