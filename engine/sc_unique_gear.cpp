...
static void register_darkmoon_card_greatness( item_t* item )
{
  ...
  // Triumvirate: proc value confirmed 120 (down from retail 300) via ItemScanner tooltip
  action_callback_t* cb = new stat_proc_callback_t( "darkmoon_card_greatness", p, max_stat, 1, 120, 0.35, 15.0, 45.0 );
                                        ...
                                        const std::string& id )
{
  ...
  else if ( name == "chuchus_tiny_box_of_horrors"         ) e = "OnAttackHit_128Crit_15%_10Dur_45Cd";  // Triumvirate: 128 (down from retail 258)
  ...
  else if ( name == "forge_ember"                         ) e = ( heroic ? "OnSpellCastHit_172SP_10%_10Dur_45Cd" : "OnSpellCastHit_115SP_10%_10Dur_45Cd" );  // Triumvirate tooltip confirmed both tiers
  ...
  else if ( name == "mirror_of_truth"                     ) e = "OnAttackCrit_333AP_10%_10Dur_50Cd";   // Triumvirate: 333 (down from retail 1000)
  ...
  else if ( name == "quagmirrans_eye"                     ) e = ( heroic ? "OnSpellCast_215Haste_10%_6Dur_45Cd" : "OnSpellCast_192Haste_10%_6Dur_45Cd" );  // Triumvirate tooltip confirmed both tiers
  else if ( name == "sextant_of_unstable_currents"        ) e = "OnSpellCrit_190SP_20%_15Dur_45Cd";
  else if ( name == "shiffars_nexus_horn"                 ) e = "OnSpellCrit_135SP_20%_10Dur_45Cd";  // Triumvirate tooltip confirmed: 135 (was using retail 225)
  else if ( name == "sundial_of_the_exiled"               ) e = "OnSpellCast_168SP_10%_10Dur_45Cd";   // Triumvirate: 168 (down from retail 590)
  else if ( name == "wrath_of_cenarius"                   ) e = "OnSpellCastHit_132SP_5%_10Dur";
  else if ( name == "meteorite_whetstone"                 ) e = ( heroic ? "OnAttackHit_177Haste_15%_10Dur_45Cd" : "OnAttackHit_170Haste_15%_10Dur_45Cd" );  // Triumvirate tooltip confirmed both tiers; flat crit rating (18/25) is base item stat, not a proc
  else if ( name == "spark_of_life"                       ) e = ( heroic ? "OnSpellCast_109MP5_10%_15Dur_50Cd" : "OnSpellCast_105MP5_10%_15Dur_50Cd" );    // Triumvirate tooltip confirmed both tiers
  else if ( name == "sonic_booster"                       ) e = "OnAttackHit_215AP_35%_10Dur_60Cd";     // Triumvirate: 215 (down from retail 430); cooldown per tooltip text ("once every minute"), retail code uses 50s ICD
  else if ( name == "crusaders_locket"                    ) e = "OnAttackHit_258Expertise_15%_10Dur_45Cd";  // Triumvirate: retail placeholder, unverified
  ...
  else if ( name == "pendulum_of_telluric_currents"       ) e = ( heroic ? "OnSpellHit_700Shadow_15%_45Cd" : "OnSpellHit_673Shadow_15%_45Cd" );  // Triumvirate tooltip confirmed both tiers; value = midpoint of tooltip's stated ranges (505-895 heroic, 485-860 normal)
  else if ( name == "vestige_of_haldor"                   ) e = "OnAttackHit_656Fire_15%_45Cd";   // Triumvirate: 656 (down from retail avg 1280); value = midpoint of tooltip's 550-762 range
  else if ( name == "dmc_death"                           ) e = "OnSpellHit_2000Shadow_15%_45Cd";  // Triumvirate: retail placeholder, unverified; value = midpoint of retail 1750-2250 range
  else if ( name == "horn_of_agent_fury"                  ) e = "OnAttackHit_1280Holy_15%_45Cd";    // Triumvirate: retail placeholder, unverified; value = midpoint of retail 1024-1536 range
  else if ( name == "dmc_berserker"                       ) e = "OnSpellHit_35Crit_3Stack_50%_12Dur";  // Triumvirate: retail placeholder, unverified; retail trigger is on dealing OR taking a spell hit, engine only supports "dealt" side

  // TBC-era items (Triumvirate raids span Classic/TBC/WotLK content) - all placeholders, unverified against Triumvirate tooltips
  ...
  else if ( name == "timbals_focusing_crystal"            ) e = ( heroic ? "OnSpellTickDamage_297Shadow_10%_45Cd" : "OnSpellTickDamage_283Shadow_10%_45Cd" );  // FIXED: was slugged "timbals_crystal", which never matches the real item name "Timbal's Focusing Crystal" -- this entry was dead code. Values now match tooltip midpoints (208-358 normal, 218-375 heroic); school not stated on tooltip, Shadow assumed (matches real WotLK item); % and Cd still unstated, assumed 10%/45s
                                      ...
                                      const std::string& id )
{
  ...
  else if ( name == "binding_light"               ) e = ( heroic ? "OnSpellCast_74SP_8Stack_20Dur_120Cd" : "OnSpellCast_66SP_8Stack_20Dur_120Cd" );      // Triumvirate: retail placeholder, unverified - part of same Ulduar hardmode reward chain as fetish/vengeance
  else if ( name == "binding_stone"               ) e = ( heroic ? "OnSpellCast_74SP_8Stack_20Dur_120Cd" : "OnSpellCast_66SP_8Stack_20Dur_120Cd" );      // Triumvirate: retail placeholder, unverified
  ...
  else if ( name == "nevermelting_ice_crystal"   ) e = "OnSpellDirectCrit_73Crit_5Stack_20Dur_180Cd_reverse";  // Triumvirate tooltip confirmed: 73 per stack, 5 stacks = 365 total (was using 184, didn't match the tooltip's stated 365-total/73-per-crit math at all)
  else if ( name == "death_knights_anguish"      ) e = "OnAttackHit_15Crit_10Stack_10%_20Dur_45Cd";  // Triumvirate: retail placeholder, unverified - stacking crit proc
