...
player_t::player_t( sim_t*             s,
    ...
    next( 0 ), index( -1 ), type( t ), level( 60 ), use_pre_potion( -1 ), tank( -1 ),  // Triumvirate: was 80 (retail default), now matches level-60 cap
    party( 0 ), member( 0 ),
...
void player_t::init_meta_gem( gear_stats_t& item_stats )
{
  ...
  {
    // was entirely missing from this codebase -- real WotLK mechanic is 1 PPM (not a flat %),
    // triggered on landing a melee or ranged auto-attack, 40s internal cooldown on top of the
    // PPM roll, 6s duration. Negative proc_chance is this engine's existing convention for "N
    // PPM" (see sc_item.cpp's "ppm" token handling), so this reuses the same weapon-speed-aware
    // PPM math that item procs like the Ashen Bands already get, rather than a flat-% approximation.
    unique_gear_t::register_stat_proc( PROC_ATTACK, RESULT_HIT_MASK, "thundering_skyflare", this, STAT_HASTE_RATING, 1, 480, -1.0, 6.0, 40.0 );
  }
}
