...
enum { ASPECT_NONE = 0, ASPECT_HAWK, ASPECT_VIPER, ASPECT_BEAST, ASPECT_DRAGONHAWK, ASPECT_MAX };  // Triumvirate: NEW Aspect of the Dragonhawk (lvl 60, +230 RAP, +18% dodge - dodge unmodeled, no defensive stats in this sim)
...
struct hunter_t : public player_t
{
    ...
    buff_t* buffs_aspect_of_the_dragonhawk;  // Triumvirate: NEW
...
struct hunter_pet_t : public pet_t
{
    ...
    virtual void init_base()
    {
        ...
        attribute_base[ATTR_STAMINA] += o->stamina() * talents.wild_hunt * 0.16;  // Triumvirate: NEW 16/32% Stamina transfer, confirmed changelog 7/1
        // FLAG: pet resource_base[RESOURCE_HEALTH] below is a fixed per-level table, not derived from ATTR_STAMINA in this fork,
        // so this line currently has no effect on pet health/DPS. Raise with Dan before assuming this "does" anything.
    ...
    virtual double composite_attack_power() SC_CONST
    {
        ...
        ap += o->composite_attack_power() * talents.wild_hunt * 0.16;  // Triumvirate: 16/32% AP transfer (was 20/40%), confirmed changelog 7/1
...
struct ranged_t : public hunter_attack_t
{
    ...
    virtual void execute()
    {
        ...
        if (result_is_hit())
        {
            ...
            if (p->active_aspect == ASPECT_HAWK || p->active_aspect == ASPECT_DRAGONHAWK)  // Triumvirate: NEW - IAotH also affects Dragonhawk
...
struct aspect_t : public hunter_spell_t
{
    ...
    int choose_aspect()
    {
        ...
        // Triumvirate: NEW - Aspect of the Dragonhawk (lvl 60) supersedes Aspect of the Hawk
        // as the default ranged/offensive aspect wherever this code would otherwise pick Hawk.
        int ranged_aspect = (p->level >= 60) ? ASPECT_DRAGONHAWK : ASPECT_HAWK;

        if (hawk_always) return ranged_aspect;
    ...
    virtual void execute()
    {
        ...
        if (aspect != p->active_aspect)
        {
            if (aspect == ASPECT_HAWK)
            {
                ...
                p->buffs_aspect_of_the_dragonhawk->expire();
                p->buffs_aspect_of_the_hawk->trigger(1, hawk_bonus);
            }
            else if (aspect == ASPECT_DRAGONHAWK)  // Triumvirate: NEW, lvl 60, flat +230 RAP (dodge component unmodeled, no defensive stats in this sim)
            {
                if (sim->log) log_t::output(sim, "%s performs Aspect of the Dragonhawk", p->name());
                p->active_aspect = ASPECT_DRAGONHAWK;
                p->buffs_aspect_of_the_viper->expire();
                p->buffs_aspect_of_the_hawk->expire();
                p->buffs_aspect_of_the_dragonhawk->trigger(1, 230.0);
            }
...
void hunter_t::init_buffs()
{
    ...
    buffs_aspect_of_the_dragonhawk = new buff_t(this, "aspect_of_the_dragonhawk", 1, 0.0);  // Triumvirate: NEW, lvl 60, flat +230 RAP
...
double hunter_t::composite_attack_power() SC_CONST
{
    ...
    ap += buffs_aspect_of_the_dragonhawk->value();  // Triumvirate: NEW, flat +230 RAP
