...
struct rogue_t : public player_t
{
    ...
    buff_t* buffs_assassins_creed;  // Triumvirate: NEW - Shadow Dance triggers +20% all damage for 12s
    buff_t* buffs_hakkars_chosen;   // Triumvirate: NEW - Mutilate stacking Nature damage buff, 5%/stack up to 5
    ...
    struct glyphs_t
    {
        ...
        int hunger_for_blood;
        int killing_spree;
        int alchemist;  // Triumvirate: was 'mutilate', reworked into Glyph of the Alchemist (Mutilate AND Envenom -5 energy)
        int preparation;
        int rupture;
        int shadow_dance;
        int sinister_strike;
    ...
    virtual double    composite_player_multiplier(int school) SC_CONST;  // Triumvirate: NEW - Assassin's Creed (Shadow Dance)
...
namespace   // ANONYMOUS NAMESPACE =========================================
{
    ...
    void rogue_attack_t::player_buff()
    {
        ...
        if (weapon)
        {
            if (p->talents.mace_specialization)
            {
                ...
                {
                    player_penetration += p->talents.mace_specialization * 0.06;
                }
            }
        ...
        player_multiplier *= 1.0 + p->talents.murder * 0.04;
    ...
    struct adrenaline_rush_t : public rogue_attack_t
    {
            ...
            rogue_attack_t("adrenaline_rush", player, SCHOOL_PHYSICAL, TREE_COMBAT)
        {
            ...
            trigger_gcd = 0;
            cooldown->duration = 120;  // Triumvirate: 120s (was 180s)
    ...
    struct backstab_t : public rogue_attack_t
    {
            ...
            rogue_attack_t("backstab", player, SCHOOL_PHYSICAL, TREE_COMBAT)
        {
            ...
            base_crit += (p->talents.puncturing_wounds * 0.10 +  // Confirmed via tooltip: Backstab gets 10/20/30% (different rate than Mutilate's 7/14/21%) - this was already correct, not a bug
    ...
    struct envenom_t : public rogue_attack_t
    {
            ...
            rogue_attack_t("envenom", player, SCHOOL_NATURE, TREE_ASSASSINATION), min_doses(1), no_buff(0)
        {
            ...
            base_cost = 35;
            if (p->glyphs.alchemist) base_cost -= 5;  // Triumvirate: NEW - Glyph of the Alchemist now also affects Envenom
    ...
    struct hunger_for_blood_t : public rogue_attack_t
    {
            ...
            refresh_at(5)
        {
            ...
            base_cost = 10.0;  // Triumvirate: 10 energy (was 15)
    ...
    // Fan of Knives =============================================================
    // Triumvirate: NEW - confirmed via tooltip (SpellID 51723). 50 energy, instant,
    // requires melee weapon, throws both weapons at all targets within 8 yards:
    // 105% weapon damage with daggers, 70% with any other weapon type. Generates
    // 1 combo point on the primary target. Learned at level 20.

    struct fan_of_knives_t : public rogue_attack_t
    {
        fan_of_knives_t(player_t* player, const std::string& options_str) :
            rogue_attack_t("fan_of_knives", player, SCHOOL_PHYSICAL, TREE_NONE)
        {
            rogue_t* p = player->cast_rogue();
            ...
            normalize_weapon_speed = true;
            adds_combo_points = true;
            aoe = true;  // hits all targets within 8 yards, approximated via adds_nearby like Whirlwind/Thunder Clap
            base_cost = 50;
        }
        ...
        virtual bool ready()
        {
            return rogue_attack_t::ready();  // "requires a valid, active combat unit target" - base ready() already implies this in this engine's single/multi-target model
        }
    };

    // Opportunity Strike ========================================================
    // Triumvirate: NEW baseline ability, learned at level 1. Instant, 50% weapon
    // damage, pierces armor, costs 20 energy, awards 1 combo point.
    // FLAG: base numbers (50% weapon dmg, 20 energy, armor pierce via base_penetration=1.0)
    // are taken directly from the changelog tooltip text; no server-side rank table
    // or spell ID was available, so this hasn't been cross-checked against SpellData.

    struct opportunity_strike_t : public rogue_attack_t
    {
        opportunity_strike_t(player_t* player, const std::string& options_str) :
            rogue_attack_t("opportunity_strike", player, SCHOOL_PHYSICAL, TREE_NONE)
        {
            rogue_t* p = player->cast_rogue();
            ...
            adds_combo_points = true;
            base_cost = 20;
            base_penetration = 1.0;  // pierces through armor
        }
    };
    ...
    struct mutilate_t : public rogue_attack_t
    {
            ...
            rogue_attack_t("mutilate", player, SCHOOL_PHYSICAL, TREE_ASSASSINATION)
        {
            ...
            base_crit += (p->talents.puncturing_wounds * 0.07 +  // Triumvirate: 7/14/21% (was 5/10/15%) - previously missed
                p->talents.turn_the_tables * 0.02);
            ...
            if (p->glyphs.alchemist) base_cost -= 5;  // Triumvirate: Glyph of the Alchemist (was Glyph of Mutilate)
        }
        ...
        virtual void execute()
        {
            ...
            if (result_is_hit())
            {
                weapon = &(player->off_hand_weapon);
                rogue_attack_t::execute();
                p->buffs_hakkars_chosen->trigger();  // Triumvirate: NEW - 5%/stack, up to 5, Nature damage on Poisons/Envenom
            }
        }
    ...
    struct rupture_t : public rogue_attack_t
    {
            ...
            rogue_attack_t("rupture", player, SCHOOL_BLEED, TREE_ASSASSINATION)
        {
            ...
            base_tick_time = 2.0;
            num_ticks = 3;
            base_multiplier *= 1.0 + (p->talents.blood_spatter * 0.18 +
                p->talents.find_weakness * 0.02 +
                p->talents.serrated_blades * 0.15 +
        ...
        virtual void execute()
        {
            ...
            if (result_is_hit())
            {
                added_ticks = 0;
                num_ticks = 3 + cp;
                if (p->glyphs.rupture) num_ticks += 4;  // Triumvirate: +8s / +4 ticks (was +4s / +2 ticks)
                update_ready();
            }
        }
    ...
    struct shadowstep_t : public rogue_attack_t
    {
            ...
            rogue_attack_t("shadowstep", player, SCHOOL_PHYSICAL, TREE_SUBTLETY)
        {
            ...
            trigger_gcd = 0;
            base_cost = 10;
            cooldown->duration = 25;  // Triumvirate: 25s (was 30s)
    ...
    struct shadow_dance_t : public rogue_attack_t
    {
            ...
            rogue_attack_t("shadow_dance", player, SCHOOL_PHYSICAL, TREE_SUBTLETY)
        {
            ...
            trigger_gcd = 0;
            cooldown->duration = 40;  // Triumvirate: 40s (was 60s)
            cooldown->duration -= p->talents.filthy_tricks * 5;  // Triumvirate: NEW - Filthy Tricks now also reduces Shadow Dance CD
        ...
        virtual void execute()
        {
            ...
            if (sim->log) log_t::output(sim, "%s performs %s", p->name(), name());
            update_ready();
            p->buffs_shadow_dance->duration = p->glyphs.shadow_dance ? 12.0 : 8.0;  // Triumvirate: base 8s (was 6s), glyph +4s (was +2s)
            p->buffs_shadow_dance->trigger();
            p->buffs_assassins_creed->trigger();  // Triumvirate: NEW - +20% all damage for 12s
        }
    };
    ...
    struct deadly_poison_t : public rogue_poison_t
    {
        ...
        virtual void execute()
        {
            ...
            {
                double chance = 0.30;
                chance += p->talents.improved_poisons * 0.05;  // Triumvirate: 5/10/15/20/25% (was 4/8/12/16/20%)
                if (p->buffs_envenom->up())
                {
                    chance += 0.20;  // Triumvirate: 20% (was 15%)
                }
    ...
    struct instant_poison_t : public rogue_poison_t
    {
        ...
        virtual void execute()
        {
            ...
            {
                double PPM = 8.57;
                PPM *= 1.0 + (p->talents.improved_poisons * 0.11) + (p->buffs_envenom->up() ? 0.75 : 0.00);  // Triumvirate: 11/22/33/44/55% (was 10/20/30/40/50%)
...
// Triumvirate: NEW - Assassin's Creed. Shadow Dance triggers a +20% all-damage
// buff for 12 seconds.

double rogue_t::composite_player_multiplier(int school) SC_CONST
{
    double m = player_t::composite_player_multiplier(school);
    if (buffs_assassins_creed->up()) m *= 1.20;
    if (school == SCHOOL_NATURE) m *= 1.0 + buffs_hakkars_chosen->current_stack * 0.05;  // Triumvirate: NEW - Hakkar's Chosen, 5%/stack, Poisons+Envenom
    return m;
}
    ...
    const std::string& options_str)
{
    ...
    if (name == "opportunity_strike") return new opportunity_strike_t(this, options_str);
...
void rogue_t::init_glyphs()
{
    ...
    for (int i = 0; i < num_glyphs; i++)
    {
        ...
        else if (n == "alchemist") glyphs.alchemist = 1;  // Triumvirate: was 'mutilate'
...
void rogue_t::init_base()
{
    ...
    initial_attack_power_multiplier *= 1.0 + (talents.savage_combat * 0.02 +
        talents.deadliness * 0.03);  // Triumvirate: 3/6/9/12/15% (was 2/4/6/8/10%)

    base_attack_expertise = talents.weapon_expertise * 6;  // FIX: was 0.05 (100x too small - base_attack_expertise is flat skill points, not a fraction). Triumvirate: 6/12 (was 5/10)
...
void rogue_t::init_buffs()
{
    ...
    buffs_adrenaline_rush = new buff_t(this, "adrenaline_rush", 1, (glyphs.adrenaline_rush ? 25.0 : 20.0));  // Triumvirate: 20s base (was 15s), glyph keeps +5s bonus
    ...
    buffs_assassins_creed = new buff_t(this, "assassins_creed", 1, 12.0);  // Triumvirate: NEW
    buffs_hakkars_chosen = new buff_t(this, "hakkars_chosen", 5, 0.0);  // Triumvirate: NEW - no expiry modeled, consumed/refreshed by rotation; flag if a duration surfaces later
