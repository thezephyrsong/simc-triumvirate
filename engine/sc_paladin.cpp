...
struct paladin_t : public player_t
{
    ...
    struct glyphs_t
    {
        ...
        int avenging_wrath;
        int consecration;
        int retribution;  // Triumvirate: was 'crusader_strike', reworked into Glyph of Retribution
        int reckoning;    // Triumvirate: NEW - Glyph of Reckoning (Hand of Reckoning no longer taunts, unimplemented - HoR itself isn't a modeled action)
        int exorcism;
        int hammer_of_wrath;
        int holy_shock;
        int holy_wrath;
    ...
    virtual double    composite_player_multiplier(int school) SC_CONST;  // Triumvirate: Sharpened (replaces Deflection)
...
namespace { // ANONYMOUS NAMESPACE ==========================================
    ...
    static void trigger_judgements_of_the_wise(action_t* a)
    {
        ...
        p->resource_gain(RESOURCE_MANA, 0.30 * p->resource_base[RESOURCE_MANA], p->gains_judgements_of_the_wise);  // Triumvirate: 30% (was 25%/0.25 in code, confirmed 30% via tooltip)
    ...
    struct paladin_attack_t : public attack_t
    {
            ...
            trigger_seal(false)
        {
            ...
            player_crit_multiplier *= 1.0 + p->talents.conviction * 0.01;  // Triumvirate: NEW - Conviction crit damage 1/2/3/4/5%
        ...
        virtual void player_buff()
        {
            ...
            if (p->talents.two_handed_weapon_spec)
            {
                ...
                {
                    player_multiplier *= 1.0 + 0.03 * p->talents.two_handed_weapon_spec;  // Triumvirate: 3/6/9% (was 2/4/6%)
                }
            }
    ...
    static void trigger_righteous_vengeance(action_t* a)
    {
        ...
        double dmg = p->talents.righteous_vengeance * 0.15 * a->direct_dmg;  // Triumvirate: 15/30/45% (was 10/20/30%)
    ...
    struct avengers_shield_t : public paladin_attack_t
    {
            ...
            paladin_attack_t("avengers_shield", p)
        {
            ...
            base_multiplier *= 0.67;  // Triumvirate: raw base Holy damage reduced 33%

            if (p->glyphs.avengers_shield) base_multiplier *= 1.5;  // Triumvirate: glyph reworked to +50% damage (was +100%/2.0x)
        }
    };
    ...
    struct crusader_strike_t : public paladin_attack_t
    {
            ...
            paladin_attack_t("crusader_strike", p)
        {
            ...
            cooldown->duration = 4;
            if (p->glyphs.retribution) cooldown->duration -= 1;  // Triumvirate: Glyph of Retribution -1s CS cooldown

            base_multiplier *= 1.0 + 0.05 * p->talents.sanctity_of_battle;
            base_multiplier *= 1.0 + p->talents.the_art_of_war * 0.06;  // FIX: was still 0.05, every other AoW application already uses Triumvirate's 6/12%
        ...
        virtual void execute()
        {
            ...
            // Triumvirate: Crusader Strike shares an interconnected cooldown state with
            // Hammer of the Righteous and Penetrating Strike (but not Hammer of Wrath).
            p->get_cooldown("hammer_of_the_righteous")->start(6);
            p->get_cooldown("penetrating_strike")->start(4.5);
        }

        virtual bool ready()
        {
            paladin_t* p = player->cast_paladin();
            if (p->get_cooldown("hammer_of_the_righteous")->remains() > 0) return false;
            if (p->get_cooldown("penetrating_strike")->remains() > 0) return false;
    ...
    struct divine_storm_t : public paladin_attack_t
    {
            ...
            paladin_attack_t("divine_storm", p)
        {
            ...
            if (p->glyphs.retribution) cooldown->duration -= 2;  // Triumvirate: Glyph of Retribution -2s DS cooldown
    ...
    struct hammer_of_the_righteous_t : public paladin_attack_t
    {
        ...
        virtual void execute()
        {
            ...
            // Triumvirate: shares an interconnected cooldown state with Crusader Strike
            // and Penetrating Strike (but not Hammer of Wrath).
            p->get_cooldown("crusader_strike")->start(p->glyphs.retribution ? 3 : 4);
            p->get_cooldown("penetrating_strike")->start(4.5);
        }
        ...
        {
            paladin_t* p = player->cast_paladin();
            if (p->get_cooldown("crusader_strike")->remains() > 0) return false;
            if (p->get_cooldown("penetrating_strike")->remains() > 0) return false;
            return paladin_attack_t::ready();
        }
    };
    ...
    struct seal_of_command_proc_t : public paladin_attack_t
    {
            ...
            paladin_attack_t("seal_of_command_proc", p, SCHOOL_HOLY)
        {
            ...
            trigger_gcd = 0;
            weapon = &(p->main_hand_weapon);
            weapon_multiplier = 0.25;  // Triumvirate: 25% weapon damage per proc (was 0.36/36%)
    ...
    struct paladin_spell_t : public spell_t
    {
            ...
            spell_t(n, p, RESOURCE_MANA, s, t)
        {
            ...
            base_crit += p->talents.holy_power * 0.02;  // Triumvirate: 2/4/6/8/10% (was 1/2/3/4/5%), confirmed via tooltip
        }
    ...
    struct divine_illumination_t : public paladin_spell_t
    {
            ...
            paladin_spell_t("divine_illumination", p)
        {
            ...
            harmful = false;
            trigger_gcd = 0;
            cooldown->duration = 150;  // Triumvirate: 150s (was 180s)
    ...
    struct hand_of_reckoning_t : public paladin_spell_t
    {
        // Triumvirate: NEW. SpellID 67485, confirmed via in-game tooltip: flat 882 Holy
        // damage, no visible AP/SP scaling text. Only one data point at 1761 AP was
        // available, so a small scaling component can't be fully ruled out - flag this
        // if parses look off relative to logs. Without Glyph of Reckoning this ability
        // only deals damage when the taunt actually lands on a target not already
        // targeting the paladin (rare/never in a normal tank-and-spank sim), so it's
        // gated to only be usable here when the glyph is active, matching the
        // changelog's "no longer taunts and can deal damage to untauntable targets."
        hand_of_reckoning_t(paladin_t* p, const std::string& options_str) :
            paladin_spell_t("hand_of_reckoning", p)
        {
            option_t options[] =
            {
              { NULL, OPT_UNKNOWN, NULL }
            ...
            trigger_gcd = 0;
            base_cost = 0;
            may_crit = false;  // not confirmed to crit; no crit-related text in tooltip

            base_dd_min = base_dd_max = 882;
        ...
        {
            paladin_t* p = player->cast_paladin();
            if (!p->glyphs.reckoning) return false;
            return paladin_spell_t::ready();
        }
    };
...
// penetrating_strike_t =====================================================
// Triumvirate: new baseline Paladin ability learned at level 1.
// Instant melee attack for 75% weapon damage. Shares cooldown with
// Crusader Strike and Hammer of the Righteous (but NOT Hammer of Wrath).

struct penetrating_strike_t : public paladin_attack_t
{
    penetrating_strike_t(paladin_t* p, const std::string& options_str) :
        paladin_attack_t("penetrating_strike", p, SCHOOL_PHYSICAL, TREE_RETRIBUTION)
    {
        may_crit = true;
        base_cost = 0;
        cooldown->duration = 4.5;  // Shares with Crusader Strike
        base_multiplier *= 0.75;
        base_multiplier *= 1.0 + p->talents.the_art_of_war * 0.06;  // Triumvirate: NEW - AoW now scales Penetrating Strike too (6/12%)
    }
    ...
    virtual void execute()
    {
        ...
        // Triumvirate: shares an interconnected cooldown state with Crusader Strike
        // and Hammer of the Righteous (but not Hammer of Wrath).
        p->get_cooldown("crusader_strike")->start(p->glyphs.retribution ? 3 : 4);
        p->get_cooldown("hammer_of_the_righteous")->start(6);
    }
...
// FLAG: "shares a cooldown with Crusader Strike/Hammer of the Righteous" is NOT actually wired up above -
// this just sets its own independent cooldown_t object of the same duration, it does not point at the same
// cooldown_t as crusader_strike_t/hammer_of_the_righteous_t. True sharing would need all three to use
// p->get_cooldown("crusader_strike") so they lock each other out. Need your call before restructuring CS/HotR's
// cooldowns, since HotR currently has its own independent 6s cooldown and this would change that for Protection sims.
...
action_t* paladin_t::create_action(const std::string& name, const std::string& options_str)
{
    ...
    if (name == "penetrating_strike") return new penetrating_strike_t(this, options_str);
...
void paladin_t::init_base()
{
    ...
    attribute_multiplier_initial[ATTR_STAMINA] *= 1.0 + talents.combat_expertise * 0.03 + talents.sacred_duty * 0.03;  // Triumvirate: Combat Expertise 3/6/9% (was 2/4/6%), Sacred Duty 3/6% (was 2/4% - previous 0.04 coefficient was a bug, gave 4/8%)
    attribute_multiplier_initial[ATTR_INTELLECT] *= 1.0 + talents.divine_intellect * 0.03;  // Triumvirate: 3/6/9/12/15% (was 2/4/6/8/10%)
    ...
    base_parry = 0.05;  // Triumvirate: Deflection reworked into Sharpened (damage talent) - no longer grants parry
    base_block = 0.05;
    initial_armor_multiplier *= 1.0 + 0.03 * talents.toughness;  // Triumvirate: 3/6/9/12/15% (was 2/4/6/8/10%)
...
void paladin_t::init_glyphs()
{
    ...
    for (int i = 0; i < num_glyphs; i++)
    {
        ...
        else if (n == "retribution") glyphs.retribution = 1;  // Triumvirate: was 'crusader_strike'
        else if (n == "reckoning") glyphs.reckoning = 1;    // Triumvirate: NEW - Hand of Reckoning now implemented, gated on this glyph for DPS relevance
        ...
        else if (n == "blessing_of_kings");  // Triumvirate: reworked to +20% effectiveness, but only affects raid recipients, not caster's own DPS - no-op by design
        else if (n == "blessing_of_might");  // Triumvirate: same as above
        else if (n == "blessing_of_wisdom");  // Triumvirate: same as above (mana return to raid, not caster DPS)
        else if (n == "cleansing");
...
void paladin_t::init_buffs()
{
    ...
    buffs_avenging_wrath = new buff_t(this, "avenging_wrath", 1, 30.0);  // FIX: was 20.0s, confirmed Triumvirate tooltip says 30 sec
    ...
    buffs_holy_shield = new buff_t(this, "holy_shield", 10, 10.0);  // Triumvirate: 10 charges (was 8)
...
void paladin_t::init_actions()
{
    ...
    if (action_list_str.empty())
    {
        ...
        else                           action_list_str += "/penetrating_strike";  // Triumvirate: NEW baseline lvl-1 filler when CS isn't talented - confirm priority w/ Dan
...
// Triumvirate: Deflection reworked into Sharpened - flat 1/2/3/4/5% global damage
// bonus, applied to all outgoing damage regardless of school.

double paladin_t::composite_player_multiplier(int school) SC_CONST
{
